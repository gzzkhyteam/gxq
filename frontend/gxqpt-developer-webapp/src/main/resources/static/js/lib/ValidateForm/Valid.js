...
$.fn.valid = function (options) {
    ...
    $(options.form).Validform({
        ...
        , datatype: {//传入自定义datatype类型，可以是正则，也可以是函数（函数内会传入一个参数）;
            ...
            "type8": function (gets, obj, curform, regxp) {
                if(gets==''){
                    return false;
                }
                if (gets > 32767 || gets < -32767) {
                    return false
                }
                return true
            },
            "ip": function (gets, obj, curform, regxp) {
            	if(!gets || gets.length<1){
            		return true;
            	}
            	var ipaddrs = gets.split(".");
            	if(ipaddrs.length != 4){
            		return false;
            	}
            	for (var i = 0; i < ipaddrs.length; i++) {
        			var ipaddr = parseInt(ipaddrs[i]);
        			if((!ipaddr && ipaddr!=0 ) || (ipaddr+"").length != ipaddrs[i].length || ipaddr > 255 || ipaddr < 0){
        				return false;
        			}
        		}
                return true
            ...
            "type16": function (gets, obj, curform, regxp) {
                if(gets==''){
                    return false;
                }
                if (gets > 2147483647 || gets < -2147483647) {
                    return false
                }
                return true
            },
            //主机管理特殊需要
            "hostmanage": function (gets, obj, curform, regxp) {
                if(gets==''){
                    return false;
                }
                var patt = new RegExp("^([\u4E00-\uFA29]|[\uE7C7-\uE7F3]|[a-zA-Z0-9_]){1,64}$");
                if (!patt.test(gets)) {
                    return false
                }
                return true
            ...
            "systemversion": function (gets, obj, curform, regxp) {
                if(gets==''){
                    return false;
                }
                var patt = new RegExp("^([\u4E00-\uFA29]|[\uE7C7-\uE7F3]|[a-zA-Z0-9_]|\\s){1,20}$");
                if (!patt.test(gets)) {
                    return false
                }
                return true
