/**
 * jQuery HengYunUI 1.1 恒运贵阳分公司 Copyright © Hengyun 2016
 */
...
function JudgeValidate(obj) {
    ...
    $(obj).find("[datacol=yes]").each(function () {
        if ($(this).attr("checkexpession") != undefined) {
            ...
            var execMethodNames = $(this).attr("checkexpession");
            var execMethodName = execMethodNames.split(",");
            for(var i=0; i<execMethodName.length;i++){
            	var isnotpass = window["is"+execMethodName](value);
                if(isnotpass){
                	Validatemsg = $(this).attr("err") + "不能为空！\n";
                    Validateflag = false;
                    tipCss($(this), Validatemsg,obj);
                    return;
                }
            }
        }
                ...
                case "NotNull":
                    {
                        if (isNotNull(value)) {
                            Validatemsg = $(this).attr("err") + "不能为空！\n";
                            Validateflag = false;
                            tipCss($(this), Validatemsg,obj); // return false;
                        }
                        break;
                    }
                case "Num":
...
function tipCss(obj, Validatemsg,element) {
	...
	var htmlstrs = '<div class="Validform_info" style="left: '+(left + 30)+'px; top: '+(top - 25)+'px; display: block;">'
        + '<span class="Validform_checktip">' + Validatemsg + '</span>'
        + '<span class="Validform_dec"><s class="Validform_dec1">&#9670;</s><s class="Validform_dec2">&#9670;</s></span></div>';
	
	obj.before(htmlstrs);
	obj.addClass("Validform_error");
	bindChangeIfNeed(obj,element);
}

function bindChangeIfNeed(obj,element){
	
	var change = obj.data('events');
	// -这里在想想 因为其把所有的事件都取消了
	obj.unbind("change");
	obj.change(function(){
		JudgeValidate(element);
	})
} 

function clearAllErrorInfo(mainEle){
	
	$(mainEle).find(".Validform_info").each(function (){
		$(this).remove();
	})
	$(mainEle).find("[datacol=yes]").each(function (){
