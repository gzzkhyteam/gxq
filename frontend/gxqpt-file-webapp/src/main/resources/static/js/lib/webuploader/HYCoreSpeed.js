...
// var fileUploadAddress="http://192.168.1.54:9785";
//var fileUploadAddress="http://192.168.1.39:9526/gydbdc-upload-webapp";
    ...
    (function( $ ){
        ...
        $(function() {
            ...
            uploader = WebUploader.create({
                ...
                chunked: false,
                chunkSize: 512 * 1024,
                server:fileUploadAddress+'/file/uploadspeed',
                // runtimeOrder: 'flash',
                ...
                // 可以用来监听进度
                    ...
                    .then(function(ret) {
                        ...
                        $.ajax({
                            type:"POST",
                            async:false,
                            headers: {"token": token,"_isAdmin": true,
                                _user_id: $.cookie("_user_id"),
                                _user_name: "",},
                            url: fileUploadAddress+'/file/uploadMd5Check',
                            data:{md5:ret},
                            success:function(data){
                                if(data==true){
                                    uploader.skipFile(file);
                                    exsistFileMd5.push(ret);
                                }else{
            ...
            $upload.on('click', function() {
                var params = "exsistFileMd5 []";
                uploader.options.formData[params] = exsistFileMd5;
                var fileLength =  uploader.getFiles().length;
                //所有文件都可秒传
                if(exsistFileMd5.length==fileLength){
                    $.ajax({
                        type:"POST",
                        headers: {
                            token:obj.token,
                        ...
                        url: fileUploadAddress+'/file/uploadspeedall',
                        data:{
                            "exsistFileMd5 []":exsistFileMd5,
                            folderId: obj.folderId
                        },
                        success:function(data){
                            //保存成功操作-------------------
                        ...
                        error:function (data) {
                            //保存失败操作------------------
                            parent.layer.alert('上传失败请联系管理原',function(){
                                parent.layer.closeAll();
                            });
                        }
                    });
