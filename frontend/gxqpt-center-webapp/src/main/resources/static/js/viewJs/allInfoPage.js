...
var num = Math.floor((height-200)/34);
if(num>10 && num<15){
    num = 10;
}else if(num>15){
    num = 15;
        ...
        ajaxHengyun({
            ...
            success:function(rows){
                if (rows.data){
                    ...
                    var warnTypeLi = $("#infoType").find(".active");
                    var warnTypeVal = $(warnTypeLi[0]).attr("id");
                    getList(parm);
                    var getTotalData = {
                        pageNo: 1,
                        pageSize:1,
...
function getTotal(parm) {
    ajaxHengyun({
        type: 'post',
        url: gateUrl+'/api/msgs/bbs/msg/getAllMsg',
        ...
        success: function (res) {
            if (res.data && res.data.list.length) {
                ...
                $.each(res.data.list, function (index, value) {
                    ...
                    str +='<td><input class="inputCheckbox" type="checkbox" name="checkbox" value="'+value.recId+'"/>' + n + '</td>\n' +
