...
function initExport(id, opts) {
	...
	function queryTableDatas(all, filter) {
		return function () {
			...
			if (all) {
				...
				postData.pageSize = param.records;
				postData.page = 1;
			}
			ajaxHengyun({
				url: param.url,
	 ...
	 */
	function generateCSV(res, filter) {
		var param = list.jqGrid("getGridParam");
		var csvStr = '';
		var newLine = '\r\n';
		param.colNames[0] = "序号";
	...
	function handleDownload(csvStr, all, filter) {
		var param = list.jqGrid("getGridParam");
		var fileName = (param.fileName || ((param.caption || "文件下载") + (all ? filter ? "(自定义)" : "(全部)" : "(第" + param.postData.page + "页)"))) + ".csv";
...
//通过名称取职务
function arrayPostionName(array,empInfo){
	if(array!=null){
		var arr = array.split("、");
		var aaa=[];
		for(var name in empInfo){
			for(var i=0;i<arr.length;i++){
				if(name == arr[i]){
					// 根据需求过滤相关人员的职务
					if(empInfo[name].postionName=='局长' || empInfo[name].postionName=='副局长' || empInfo[name].postionName=='调研员' || empInfo[name].postionName=='副调研员'){
						aaa.push(name + empInfo[name].postionName);
					}else{
						aaa.push(name);
					}
				}
