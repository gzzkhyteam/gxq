...
$.fn.valid = function (options) {
    ...
    //$(options.form).submit();
    //alert($(this).attr('id'));
    //options.datatype = $(this).attr('data-type');
    //});
    ...
    $(options.form).Validform({
        ...
        , datatype: {//传入自定义datatype类型，可以是正则，也可以是函数（函数内会传入一个参数）;
            ...
            "integeNum": function (gets, obj, curform, regxp) {
                if(!gets){
                    return true;
                }
                var reg = /^[0-9]\d*$/;
            ...
            "integezs": function (gets, obj, curform, regxp) {
                var reg = /^[1-9]\d*$/;
                if (!reg.test(gets)) {
                    return false;
                } else {
            ...
            "contactTel": function (gets, obj, curform, regxp) {
                var reg = /^(0\d{2,3}-?\d{7,8})$|^(13|14|15|17|18)\d{9}$/;
                if (!reg.test(gets)) {
                    return false;
                } else {
                    return true;
            ...
            "fileReg": function (gets, obj, curform, regxp) {
                var reg = /^[a-zA-z]([0-9a-zA-z-_]*)$/;
                if (!reg.test(gets)) {
                    return false;
                } else {
                    return true;
            ...
            "fileRegLong": function (gets, obj, curform, regxp) {
                var reg = /^[a-zA-z]([0-9a-zA-z-_]*)$/;
                if (!reg.test(gets)) {
                    return false;
                } else {
