var fileUploadAddress=_GATE_URL+"/api/admin/gxqpt/org";
 ...
 */
 ...
 function delFile(com,fileId){
     parent.layer.confirm("温馨提示：删除后不可恢复,确定要删除吗？",{btn: ['确定','取消'],shade: false },function(index) {
         $.ajax({
             url:fileUploadAddress+"/attachment/deleteFile.do",
             type:'post',
             data:{"id":fileId},
             dataType:"json",
...
function downFile(filePath,fileName){
	window.location.href=fileUploadAddress+"/attachment/downloadByPath.do?filePath="+filePath+"&fileName="+fileName;
}
function downFiles(busId,busType,fileName){
	window.location.href=fileUploadAddress+"/attachment/downloadFiles.do?busId="+busId+"&fileName="+encodeURIComponent(fileName)+"&busType="+encodeURIComponent(busType);
}
		 ...
		 $.each(data.value, function (i, item) {
			 if(item.busType==busType){
				...
				if(item.fileType=='jpg' || item.fileType=='JPG' || item.fileType=='png' || item.fileType=='PNG'){
					...
					html +="src="+fileUploadAddress+"/attachment/downloadByPath.do?filePath="+item.filePath+"&fileName="+item.oldName+"/>";	
	...
	(function( $ ){
	    ...
	    $(function() {
	        ...
	        uploader = WebUploader.create({
	            ...
	            chunked: false,
	            chunkSize: 512 * 1024,
	            server:fileUploadAddress+'/saveOrgs',
	            // runtimeOrder: 'flash',

	            accept: {
	                 //title: 'mb格式文件',
	            	extensions: obj.fileType
	                // mimeTypes: 'mb/*'
	             },
	            ...
	            fileSingleSizeLimit: 50 * 1024 * 1024,    // 100 M
	            auto:true //自动上传
	        }) ;
	        // 拖拽时不接受 js, txt 文件。
	        ...
	        uploader.onError = function( code) {//Q_TYPE_DENIED
	            //alert( 'Eroor: ' + code );
	        	if(code=='Q_TYPE_DENIED'){
	        		top.layer.confirm(obj.tipMsg,{
	        			  title:'温馨提示',
	        			  shade: [0.4,'#000'],
	        			  btn: ['确定'] //按钮
	        			});
                ...
                if(code=="Q_EXCEED_SIZE_LIMIT"){
                    top.layer.confirm("单个文件大小不能超过50m！",{
                        title:'温馨提示',
                        shade: [0.4,'#000'],
                        btn: ['确定'] //按钮
                    });
                ...
                if(code=="F_EXCEED_SIZE"){
                    top.layer.confirm("一次最多只能上传50m大小的文件！",{
                        title:'温馨提示',
                        shade: [0.4,'#000'],
                        btn: ['确定'] //按钮
                    });
