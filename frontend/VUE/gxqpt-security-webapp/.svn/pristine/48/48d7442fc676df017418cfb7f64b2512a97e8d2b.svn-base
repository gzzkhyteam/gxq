          ...
          <FormItem label="部门">
              ...
              style="width:180px">
                ...
                {{item.name}}
					...
					<FormItem>
						<Button type="primary" @click="getPersonData">查看分析</Button>
					</FormItem>
				</Form>
			</Card>
...
export default {
  ...
  computed: {
    ...
    departmentList () {
      ...
      if (vm.formData.unitId) {
        for (var i = 0; i < vm.allUnitAndDepartment.length; i++) {
          if (vm.allUnitAndDepartment[i].id == vm.formData.unitId) {
            return vm.allUnitAndDepartment[i].dpms
          }
        }
      }
      return []
  ...
  methods:{
    ...
    getUnitData() {
      const vm = this
      api(superviseApiList.findOrgDpmIdenti).then((res) => {
        if(res.data.errcode == 0) {
          vm.allUnitAndDepartment = res.data.data
          const unitList = [];
          vm.allUnitAndDepartment.map(item => {
            unitList.push({
              id: item.id,
              name: item.name,
        ...
        if(res.data.errcode == 0) {
          ...
          res.data.data.map((data, idx) => {
            subArr.push({
              ...
              countSize: "#50",
              introText: data.name,
              link: `/personnelAnalyzeView/${data.id}`
            })
  ...
  watch: {
    departmentList () {
      if (this.departmentList.length > 0) {
        this.formData.departmentId = this.departmentList[0].id
      } else {
