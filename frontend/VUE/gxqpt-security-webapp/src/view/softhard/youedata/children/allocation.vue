                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: (h, params) => {
                            ...
                            return create('Input', {
                                class:'server-list',
                                ref:`serverId${params.index}`,
                                props: {
                                    clearable:true,
                                    value: params.row.serverId
                                },
                                on: {
                                    "on-blur": function (value) {
                                        let reg=/^[0-9]+$/;
                                        if(value.currentTarget.value && !reg.test(value.currentTarget.value)){
                                            vm.$Message.error('请输入正确的服务器ID！');
                                            vm.$refs[`serverId${params.index}`].$refs.input.value = ''
                                            return false;
                                        }
                                        let item = vm.subData.dealServer;
                                        for(var i = 0;i<item.length;i++){
                                            if(item[i].parentId == params.row.parentId){//判断有相同键值时候
                                                item[i].serverId = value.currentTarget.value;
                                                break;
                                            }else if(!item[i].parentId){//判断没有键值时候注入数据
                                                item[i].parentId = params.row.parentId;
                                                item[i].serverId = value.currentTarget.value;
                            ...
                            return create('Input', {
                                ...
                                on: {
                                    "on-blur": (value) => {
                                        ...
                                        if(value.currentTarget.value && !reg.test(value.currentTarget.value)){
                                            vm.$Message.error('请输入正确的服务器Ip！');
                                            vm.$refs[`serverIp${params.index}`].$refs.input.value = ''
                                            return false;
                                        }
                                        let item = vm.subData.dealServer;
                                        for(var i = 0;i<item.length;i++){
                                            if(item[i].parentId == params.row.parentId){//判断有相同键值时候
                                                item[i].ip = value.currentTarget.value;
                                                break;
                                            }else if(!item[i].parentId){//判断没有键值时候注入数据
                                                item[i].parentId = params.row.parentId;
                                                item[i].ip = value.currentTarget.value;
    ...
    methods:{
        ...
        tableCreate(item,index){//表格整理然后显示；
            ...
            for(var i = 0;i<item.applyCount;i++){
                ...
                for(var j =0;j<item.disks.length;j++){
                    ...
                    tableData[i].diskProperty[j].property = item.disks[j].diskName + '(' + item.disks[j].diskSize + 'GB)';//增加单位名称
                }
            }
            return tableData
        ...
        submit(){//提交
            ...
            if(this.dataDeal(this.subData)){
              const dealServer = this.subData.dealServer
              const array = []
              for(const item of dealServer){
                const obj = {}
                obj.ip = item.ip
              ...
              const checkIp = {}
              checkIp.dtos = array
              api(softhardApiList.checkIp,checkIp).then((res) => {
                  if(res.data.errcode == 0){
                    if(res.data.data.length){
                      ...
                      for(const item of data){
                        str = 'ip：'+ item.ip + '已存在' +'<br/>' 
                        msg += str 
                      }
                      this.$Message.error(msg,3000);
                  }else{
        ...
        dataDeal(data){
            ...
            for(var index = 0;index<data.dealServer.length;index++){
                ...
                let len = this.applyData[j].data[0].diskProperty.length;//获取长度
                if(item.disks.length != len){//判断长度是否正确
                    ...
                    for(var idx = 0;idx<item.disks.length;idx++){
                        ...
                        items.diskName = this.applyData[j].disks[idx].diskName;
                        items.diskSize = this.applyData[j].disks[idx].diskSize;
                        items.diskType = this.applyData[j].disks[idx].diskType;
                    }
                }
