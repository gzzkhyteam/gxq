...
window.hy_auth_login=function (){
	...
	loginController.getLoginUrl = function (){
		...
		// var index = 0;
		// for(let i=0;i<3 && url.length > index;i++){
		// 	let indexTemp = url.indexOf("/",index+1);
		// 	if(indexTemp >= 0){
		// 		index = indexTemp;
		// 	}
		// }
		// debugger
		// var protocol_ip_port = url.substring(0,index);
		// debugger
		// protocol_ip_port += "/";
		var protocol_ip_port = window.location.protocol + '//' + window.location.host + '/';
		var path = this.getContentPath();
		var service = '';
		if(path=="gxqpt-softhard"){//软硬件管理平台
			service = protocol_ip_port + loginConfig.softhard_local_service_url;
		}else if(path=="gxqpt-security"){//安全保障平台
			service = protocol_ip_port + loginConfig.security_local_service_url;
		}else if(path=="gxqpt-supervise"){//统一监管平台
			service = protocol_ip_port + loginConfig.supervise_local_service_url;
		}else if(path=="gxqpt-operation"){//运维管理平台
			service = protocol_ip_port + loginConfig.operation_local_service_url;
		}else if(path=="gxqpt-warn"){//预警系统
			service = protocol_ip_port + loginConfig.warn_local_service_url;
		}
