...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		multiSort = function(iCol, obj, sor ) {
			...
			manage = sort.lastIndexOf(ts.p.sortorder);
			sort = sort.substring(0, manage);
			ts.p.sortname = sort;
			*/
		},
...
//		重新设置hDiv的宽度
		...
		if(ts.p.pager){
			...
			$(ts.p.pager).css({width: "auto"}).addClass(getstyle(stylemodule,'pagerBox', true,'ui-jqgrid-pager')).appendTo(eg);
...
//				$(grid.ubDiv).addClass( tbstyle + " ui-userdata-bottom").attr("id","tb_"+this.id).insertAfter(grid.hDiv).width(grid.width - bstw);
				$(grid.ubDiv).addClass( tbstyle + " ui-userdata-bottom").attr("id","tb_"+this.id).insertAfter(grid.hDiv).width("auto");
				if(hg)  {$(grid.ubDiv).hide();}
			} else {
				// 重新设置toolbar的高度
//				$(grid.uDiv).width(grid.width - bstw).addClass( tbstyle + " ui-userdata-top").attr("id","t_"+this.id);
				$(grid.uDiv).width("auto").addClass( tbstyle + " ui-userdata-top").attr("id","t_"+this.id);
			}
...
//			重新设置顶部分页宽度
			$(grid.topDiv).addClass(getstyle(stylemodule, 'toppagerBox', true, 'ui-jqgrid-toppager')).width("auto").insertBefore(grid.hDiv);
