$(function () {
    ...
    function initList () {
        ...
        $("#list").jqGrid({
            ...
            url: _GATE_URL + '/api/sms/publisher/list',
            datatype: "json",
            contentType : 'application/json',
            serializeGridData:function(postData){
                return JSON.stringify(postData);
            },
            colNames: ['姓名', '登录账号', '所在部门', '状态', '操作'],
            colModel: [{
                name: 'name',
                index: 'name'
            }, {
    ...
    function doAct(idx){
        ...
        parent.layer.confirm("确定"+(isOpen?"取消拉黑":"拉黑")+"吗？",{
            ...
            ajaxHengyun({
                ...
                data: JSON.stringify({
                    appId: $('#appId').val(),
                    depatment: data.maindeptname,
                    userAccount: data.nickname,
                    userId: data.gxqptEmpId,
                    userName: data.name,
                    cancel: isOpen ? 1 : 0
                }),
                contentType: 'application/json',
                url: _GATE_URL + '/api/sms/publisher/pullblack',
