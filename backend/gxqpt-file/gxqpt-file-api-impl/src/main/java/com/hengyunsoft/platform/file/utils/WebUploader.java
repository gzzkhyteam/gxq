...
public class WebUploader {
     ...
     */
    public com.hengyunsoft.platform.file.entity.file.po.File md5Check(String md5) {
     ...
     */
    public String chunksMerge(String folder, String fileName, int chunks, String md5, String path) {
        //合并后的目标文件
        String target;
        ...
        if (chunks == this.getChunksNum(path + "/" + folder)) {
            ...
            try {
                ...
                List<File> files = new ArrayList<>(Arrays.asList(this.getChunks(path + "/" + folder)));
        ...
        com.hengyunsoft.platform.file.entity.file.po.File file = this.md5Check(md5);
        if (file == null) {
            log.error("文件[签名:" + md5 + "]数据不完整，可能该文件正在合并中");
            this.setErrorMsg("数据不完整，可能该文件正在合并中");
            return null;
        }
        target = Paths.get(file.getAbsolutePath(), file.getFilename()).toString();
        return target;
    }
     ...
     * @return
     */
    public File getReadySpace(FileUploadDTO info, String path) {
