...
public class BbsOpenMessageApiImpl   {
     ...
     * @param
     */
    @ApiOperation(value = "保存消息到指定的用户集合", notes = "保存消息到指定的用户集合")
    @ApiResponses({
    ...
    @RequestMapping(value = "/saveMsgToUser", method = RequestMethod.POST)
    public Result<BbsAllMainMsgSaveResDTO> saveMsgToUser(@RequestBody BbsAllMainMsgDTO bammDTO){

        //校验
        if(bammDTO == null){
            return Result.fail("入参实体DTO实体不能为空！！");
        }
        Result<BbsAllMainMsgSaveResDTO> res = null;
        try {
            BbsAllMainMsgSaveReqDTO dto = new BbsAllMainMsgSaveReqDTO();
            dto.setBbsAllMainMsgDTO(bammDTO);
            dto.setHandlerUsers(bammDTO.getHandlerUserIds());
            dto.setLookUsers(bammDTO.getLookUserIds());
            dto.setRecvType(BbsAllMainMsgSaveReqDTO.PERSON_RECV_TYPE);
            res =  bbsBizCommon.saveMsgCommon(dto);
        } catch (Exception e) {
     ...
     * @param
     */
    @ApiOperation(value = "保存只需要查看类的信息到指定的角色", notes = "保存只需要查看类的信息到指定的角色")
    @ApiResponses({
            @ApiResponse(code = 75501, message = "业务id不为空"),
            @ApiResponse(code = 75502, message = "业务类型不为空"),
            @ApiResponse(code = 75503, message = "客户端标识不为空"),
            ...
            @ApiResponse(code = 75517, message = "接收人是否需要处理不为空"),
            @ApiResponse(code = 75523, message = "用户类型不为空"),
            @ApiResponse(code = 75524, message = "应用角色ID不能为空"),
            @ApiResponse(code = 75525, message = "应用角色编码不能为空"),
    })
    @RequestMapping(value = "/saveLookMsgToRole", method = RequestMethod.POST)
    public Result<BbsAllMainMsgSaveResDTO> saveLookMsgToRole(@RequestBody BbsAllMainMsgDTO bammDTO) {
        ...
        Result<BbsAllMainMsgSaveResDTO> res = null;
        BizAssert.assertNotEmpty(MsgExceptionCode.BBS_MESSAGE_APP_ROLE_ID_EMPTY, bammDTO.getRoleAppId());
        BizAssert.assertNotEmpty(MsgExceptionCode.BBS_MESSAGE_APP_ROLE_CODE_EMPTY, bammDTO.getRoleCode());
        try {
            BbsAllMainMsgSaveReqDTO dto = new BbsAllMainMsgSaveReqDTO();
            dto.setBbsAllMainMsgDTO(bammDTO);
            dto.setRoleAppId(bammDTO.getRoleAppId());
            dto.setRoleCode(bammDTO.getRoleCode());
     ...
     * @param
     */
    @ApiOperation(value = "保存只需要处理类的消息到指定的角色", notes = "保存只需要处理类的消息到指定的角色")
    @ApiResponses({
            @ApiResponse(code = 75501, message = "业务id不为空"),
            @ApiResponse(code = 75502, message = "业务类型不为空"),
            @ApiResponse(code = 75503, message = "客户端标识不为空"),
    ...
    @RequestMapping(value = "/saveHandlerMsgToRole", method = RequestMethod.POST)
    public Result<BbsAllMainMsgSaveResDTO> saveHandlerMsgToRole(@RequestBody BbsAllMainMsgDTO bammDTO) {
