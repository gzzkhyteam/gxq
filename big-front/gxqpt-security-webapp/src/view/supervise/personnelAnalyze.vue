              ...
              @on-change="getDepartData">
                ...
                {{item.name}}
					...
					<FormItem label="部门">
						...
						<Button v-if="checkButton('supervise_Analysis_person_view')" type="primary" @click="getPersonData">查看分析</Button>
					</FormItem>
				</Form>
			</Card>
...
export default {
  ...
  methods:{
    ...
    getDepartData(isOnChange){//获取所有部门
      const vm = this
      api(superviseApiList.findDpmByOrg,{systemCode:'gxqpt',orgId:vm.formData.unitId}).then((res) => {
        if(res.data.errcode == 0) {
          this.departmentList = res.data.data;
          vm.formData.departmentId = this.departmentList[0].id
          vm.$nextTick(() => {
