...
<script>
	...
	export default {
					...
					},
					{
						...
						render: (h, params) => {
							const capacity = h('Button', {
								...
								style: {
									display:this.checkButton('hardware_resources_manage_kr')?'inline-block':'none'
								},
								on: {
									click: () => {
							...
							const free = h('Button', {
								...
								style: {
									display:this.checkButton('hardware_resources_manage_sf')?'inline-block':'none'
								},
								on: {
									click: () => {
										this.gotoCtrl(2,params.row);										
		...
		mounted() {
			this.getCpuResManage(); //查询服务分配管理分页
		...
		methods: {
			...
			pageChange(num) { //页码改变的回调
				...
				this.getCpuResManage();
			},
			changePageSize(num) { //切换每页条数时的回调
				this.pageOption.pageSize = num;
				this.getCpuResManage();
			},
			getCpuResManage() { // 查询服务分配管理分页
				this.data = [];
				var formData = {
				...
				api(softhardApiList.getCpuResManage, formData).then((res) => {
