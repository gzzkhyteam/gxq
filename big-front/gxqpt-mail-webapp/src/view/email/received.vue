		...
		<Layout>
			<Content>
				<Breadcrumb>
					...
					<BreadcrumbItem>收件箱<span v-if="total">(共{{total}}封)</span></BreadcrumbItem>
					<div class="rt">
				...
				<Card>
					<Form class="seachForm" ref="formQueryData" :model="formQueryData" inline :label-width="80">
						...
						<FormItem label="标记为：">
							<Select class="w100" ref="maikType" v-model="formQueryData.type" clearable @on-change="signFun">
	...
	export default {
		...
		methods: {
			pageChange(num) { //页码改变的回调
				...
				this.mailId = "";
				this.fromVal = [];
				this.isReadFun(this.formQueryData.isRead);
			},
          ...
          if (type === 'thorough') {
			...
			isReadFun(val) { //转态选择
				...
				if(this.formQueryData.isRead==val){
					isReadData.pageNo=this.pageOption.current;
				}else{
					this.formQueryData.isRead=val;
			...
			receiveSee(data) {
				...
				api(emailApi.receiveSee, data).then((res) => {
					if(res.status == 200 && res.data.data.records) {
					  ...
					  _this.data = [];
					  _this.pageOption.pageSize = 20;
            _this.pageOption.total = 0;
            _this.pageOption.current = 1;
					}
				}, (err) => {})
			},
			forward(){//转发
