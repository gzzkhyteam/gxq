<template>
    <div class="container">
        ...
        <mu-list>
            ...
            <inputModel v-if="dateLength" name="加班时长" :icon="!icon" :req="!icon" :content="dateLength"></inputModel>
            <inputModel @test='getContent' name="加班原因" :clear="clear" :icon="!icon" :req='false' textare></inputModel>
            <div class="mu-item">
                <examination backUrl='OvertimeForm' :chance="chanceApproval"></examination>
            </div>
            <div class="mu-item">
                <Approver backUrl='OvertimeForm' :chance="chance" :req='false'></Approver>
            </div>
            <div class="mu-item">
...
export default {
    ...
    activated(){
        ...
        eventBus.$on('chance', function(data){//抄送人回调
            let chanceItem = JSON.parse(data);
        ...
        if(this.clear){//清空其他值
            ...
            this.chance = [];
            this.dateLength = '';
            this.$store.state.applyData.content = '';
            this.getHistoryData();//获取历史抄送人和审批人
        }
    },
    methods:{
        getContent(val){//获取输入内容
            this.$store.state.applyData.content = val;
        },
        countDateLength(){
        ...
        count(){
            this.$store.state.applyData.aprType = "overtime";
        ...
        getHistoryData(){
            api(ApiList.getDefaultPerson,{aprType:"overtime"})
            .then((res)=>{
                if(res.data.errcode==0){
