...
export default {
  ...
  methods: {
    initPersonnelCategory() {
      if(this.indicator.length==0){
        this.indicator = [0,0,0,0,0,0,0,0];
        this.nameList = ['暂无数据'];
    	};
      // 基于准备好的dom，初始化echarts实例
      if (!this.personnelCategory) {
        this.personnelCategory = this.$echarts.init(document.getElementById('personnelCategory'))
      }
      const option = {
        backgroundColor: '#fff',
        title: {
      ...
      // 使用刚指定的配置项和数据显示图表。
      this.personnelCategory.setOption(option)
    },
    count(){//整理数据
      let obj = {};
