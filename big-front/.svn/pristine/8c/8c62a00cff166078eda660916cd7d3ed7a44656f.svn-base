...
window.SimpleCalendar = function () {
    //构造函数
    ...
    function SimpleCalendar(query, options) {
        ...
        this._defaultOptions = {
            ...
            showHoliday: true, //休假
            showOverTime: true, //加班
            showFestival: true, //节日
            showBackup: true, //备份监控
    ...
    _createClass(SimpleCalendar, [{
        ...
        value: function update() {
            ...
            this.updateVocation(year, month);
        ...
        value: function getYearMonth() {
            var year = document.querySelector('#calendar .sc-select-year').value;
            var month = document.querySelector('#calendar .sc-select-month').value;
            return year + '-' + month;
        }
        ...
        key: 'updateVocation',
        value: function updateVocation(year, month, type) {
            var options = this._options;
            var calendar = this;
            var isenable;//是否启用
            var daysElement = this.arrayfrom(this.container.querySelectorAll('.sc-item'));
            ...
            if (options.showHoliday) {
                ...
                var data = function () {
                    if (type == 1) {//休假
                        isenable = calendar.languageData.vocation['data_' + year]['isenable'];
                        return calendar.languageData.vocation['data_' + year]['data'];
                    } else if (type == 2) {//加班
                        isenable = calendar.languageData.overTime['data_' + year]['isenable'];
                        return calendar.languageData.overTime['data_' + year]['data'];
                    }
                }();
                if (data) {
                    daysElement.forEach(function (v, i) {
                        ...
                        if (data.indexOf(currentmonth + '-' + day) >= 0) {
                            isenable || v.classList.add('sc-disabled');
                            if (type == 1) {
                                v.classList.add('sc-vocation');
                            } else if (type == 2) {
                                v.classList.add('sc-overtime');
                            }
                        }
                    });
                }
        ...
        key: 'setVocation',
        value: function setVocation(year, month, holidays, type, isenable) {
            var arr = [];
            $.each(holidays, function (index, value) {
                arr.push(value);
            })
            if (type == 1) {
                this.languageData.vocation['data_' + year] = {isenable: isenable, data: arr};
            } else if (type == 2) {
                this.languageData.overTime['data_' + year] = {isenable: isenable, data: arr};
            }
            this.updateVocation(year, month, type);
        }
        //重置节假日信息
    }, {
        key: 'resetVocation',
        value: function resetVocation() {
            $.each(this.container.querySelectorAll('.sc-item'), function (index, value) {
                value.classList.remove('sc-vocation', 'sc-overtime', 'sc-disabled');
            })
        }
        ...
        value: function updateEvent() {
            ...
            selectYear.onchange = function () {
                ...
                var y = this.value;
                calendar._defaultOptions.callBack(y, m);
                calendar.update(m, y);
            };
            ...
            yearadd.onclick = function () {
                ...
                calendar._defaultOptions.callBack(currentyear, currentmonth);
            ...
            returntoday.onclick = function () {
                ...
                calendar._defaultOptions.callBack(selectYear.value, selectMonth.value);
                calendar.update();
            };
        }
...
SimpleCalendar.prototype.languageData = {
    ...
    vocation: {
        // data_2016: {isenable:true,data:['1-1', '1-2', '1-3', '2-7', '2-8', '2-9', '2-10', '2-11', '2-12',, '10-6', '10-7']}
    },
    overTime: {
        // data_2018:{isenable:true,data:['1-1', '1-2', '1-3', '2-7', '2-8', '2-9', '2-10', '2-11', '2-12',, '10-6', '10-7']},
    }
};
