<template>
	<mu-container>
		...
		<mu-card style="width: 100%;margin: 0 auto;">
			<mu-card-title :title="notice.title" :sub-title="notice.appName"></mu-card-title>
...
<script>
	...
	export default {
		...
		mounted(){
			...
			this.getMsgDetail();
			if(!this.$route.params.readIs){
				this.updateMsgsReaded();
			}
		},
		methods:{
			getMsgDetail(){
				api(ApiList.getMsgDetail,{msgId:this.$route.params.msgId})
				.then((res)=>{
					console.log(res);
