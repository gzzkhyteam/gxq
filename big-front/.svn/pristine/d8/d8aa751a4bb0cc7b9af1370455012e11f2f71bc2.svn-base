        ...
        <Form ref="createForm" :model="createForm" :rules="createRule" :label-width="90">
          ...
          <FormItem label="文档上传" prop="documentId">
            <hyUpload
              ...
              <div v-if="editA">
                已上传文件：
                <a :href="'/api/file/p/file/down?id='+createForm.documentId+'&userId='+createForm.userId+'&token='+createForm.token"><button type="button" class="ivu-btn ivu-btn-primary ivu-btn-small"><span>下载</span></button></a>
              </div>
          </FormItem>
          ...
          const fileEdit = h('Button', {
            ...
            on: {
              click: () => {
                ...
                vm.createForm.userId = userId
                vm.createForm.token = token
                vm.isEdit = true
                vm.createModal = true
                vm.title = '修改文档'
                vm.editA = true
